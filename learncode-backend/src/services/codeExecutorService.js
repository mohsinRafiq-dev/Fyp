...
class CodeExecutorService {
  ...
  async runInDocker(code, language, sessionDir, input) {
    ...
    const imageName = `learncode-ai-${language}-${Date.now()}`;
    const containerName = `learncode-ai-container-${Date.now()}`;
    ...
    try {
      ...
      const runCommand = input
        ? `docker run --rm --name ${containerName} --memory=128m --cpus=0.5 --network=none ${imageName} < ${path.join(
            sessionDir,
            "input.txt"
          )}`
