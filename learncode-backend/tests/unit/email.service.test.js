...
describe("Email Service Tests", () => {
  ...
  describe("Send Verification OTP", () => {
    ...
    test("should handle send email errors", async () => {
      ...
      await expect(
        emailService.sendVerificationOTP("user@example.com", "123456")
    ...
    test("should not send email if service not available", async () => {
      ...
      await expect(
        emailService.sendVerificationOTP("user@example.com", "123456")
      ).rejects.toThrow("Email service is not available");
  ...
  describe("Send Password Reset OTP", () => {
    beforeEach(async () => {
      await emailService.initialize();
        ...
        subject: 'Reset Your Password - LearnCode AI',ccessfully", async () => {
      const email = "user@example.com";
      const otp = "654321";
      const name = "Test User";
    ...
    test("should handle send email errors", async () => {
      ...
      await expect(
        emailService.sendPasswordResetOTP("user@example.com", "654321")
